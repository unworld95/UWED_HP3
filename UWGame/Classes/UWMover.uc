...
//var(Reaction) class <Projectile> vulnerableToClass; //Duplicate Below
...
{
  if ( bCallHandleSpellOnHPawnOwner && (Owner != None) && Owner.IsA('HPPawn') && Other.IsA('baseSpell') )
  {
    HPPawn(Owner).HandleSpell(baseSpell(Other),Other.Location,Instigator);
  }
  if ( ClassIsChildOf(vulnerableToClass,Class'RetractumSpell') )
  {
    GotoState('StateRetractum');
    TriggerOther = Other;
    TriggerInst = Instigator;
    HPHeroPawn(Instigator).DoPreRetractum(self);
    if (  !bTriggerAfterTime )
    {
      Super.Activate(TriggerOther,TriggerInst);
    }
    return;
  }
...
state StateRetractum
{
  event OnEvent (name Event)
  {
    if ( Event == 'CasterFinishedPrep' )
    {
      if ( bTriggerAfterTime )
      {
        Activate(TriggerOther,TriggerInst);
      }
      TriggerOther.Destroy();
      TriggerInst.OnEvent('Release');
    }
  }
...
{
  if ( Other.IsA('baseSpell') )
  {
    P = baseSpell(Other).spellCaster;
  }
  Super.Trigger(Other,P);
}

function bool iHandleReTriggerDelay( actor Other )
{
	if ( Level.TimeSeconds - TriggerTime < ReTriggerDelay && TriggerTime >= 0 )
		return true;
...
function bool HandleReTriggerDelay (Actor Other)
{
  bProjTarget = False;
  fHandleRetriggerDelayTime = Level.TimeSeconds;
  return iHandleReTriggerDelay(Other);
}

function Tick (float dt)
{
  Super.Tick(dt);
  if ( (fHandleRetriggerDelayTime != 0) && (Level.TimeSeconds - fHandleRetriggerDelayTime >= ReTriggerDelay) )
  {
    fHandleRetriggerDelayTime = 0.0;
    bProjTarget = True;
  }
}
*/
